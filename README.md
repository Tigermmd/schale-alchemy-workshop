...
未按国服进度实装的学生只按礼物规划，不计入日程和咖啡厅摸头。礼包参考单独放在“礼包性价比”页面，不会混入免费资源规划。
...
制约解除作战可以选择预设楼层，也可以填写实际通关楼层。周期资源的来源和计算口径都可以在页面中展开查看。
...
Agent 只能提出规划变更。所有变更都需要玩家确认后才会由网页本地应用；库存、购买记录和网页代码不会交给 Agent 直接修改。API Key 只保存在本机 Harness 进程内存中，不写入浏览器本地存储、导出文件、URL 或日志。启用 Agent 后，当前规划、具体库存数量、礼物盒、制造/合成石、周期资源配置、礼包计划、预留、资源计入记录、礼包快照、本地计算结果和对话会发送到你填写的第三方模型服务；网页计算已完成的结果会随上下文发送，Agent 不会直接调用网页函数。不使用 Agent 时不会发送这些数据。
...
- 启动石只计入第一阶段，第二、第三阶段使用的其他材料不计入启动石消耗；
...
学生 ID 使用跨服务器一致的 ID。已实装学生可以获得日程和咖啡厅好感；未实装或状态未知的学生只使用礼物相关资源。共享的日程、咖啡厅和周期资源只分配给当前主目标。
...
国服资源还使用官方公告、中文社区资料和玩家提供的规划口径。每项数据都保留来源和适用范围；页面允许修改可配置的玩家输入值。

详细字段说明见 [`relationship_data/README.md`](relationship_data/README.md)。

## 本地数据与隐私

库存、规划、语言和部分页面设置保存在浏览器本地。仓库不提供账号系统；不使用 Agent 时不会上传个人库存。启用 Agent 后，发送给模型服务的数据范围见上方 Agent 说明。

使用 Agent 时，API Key 只进入本机 Harness 进程内存。分享项目或提交代码前，请检查个人库存 JSON、日志、截图和其他凭据没有进入仓库。
