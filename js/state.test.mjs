...
assert.equal(text("zh_cn", "pageTitle"), "养成规划工作台");
assert.equal(text("en", "pageTitle"), "Relationship Planner Workbench");
assert.equal(text("ja", "pageTitle"), "絆育成ワークベンチ");
assert.equal(text("en", "documentTitle"), "Schale Alchemy Workshop");
assert.equal(text("ja", "documentTitle"), "シャーレ錬金工房");
...
assert.match(giftBoxHtml, /国服概率未填/);
...
assert.match(userConfirmedGiftBoxHtml, /按当前填写值等概率计算/);
