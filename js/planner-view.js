...
import { calculatePlanningSummary } from "./planning-summary.js?v=dashboard-20260814-rebuild-v45";
...
export function renderWorkbenchTabs({ locale, active }) {
  ...
  return `<nav class="workbench-tabs" aria-label="${escapeHtml(t(locale, "workbenchNavigation"))}">${tabs.map(([id, label, icon]) => `<button type="button" class="workbench-tab ${id === active ? "is-active" : ""}" data-workbench="${id}" data-nav-icon="${escapeHtml(icon)}" aria-current="${id === active ? "page" : "false"}">${escapeHtml(label)}</button>`).join("")}</nav>`;
}
