...
import { calculateInventorySummary, mapPeriodicResource } from "./inventory-state.js?v=dashboard-20260814-rebuild-v47";
import { localizedName, text as t } from "./i18n.js?v=dashboard-20260814-rebuild-v47";
import { formatExp, formatSmartQuantity } from "./render.js?v=dashboard-20260814-rebuild-v47";
...
export function renderInventoryWorkspace({ data, state, locale, localization, filters = {}, notice = "" }) {
  ...
  const choiceBox = summaryValue(summary.giftBoxes, "100008");
  const randomPurpleBox = summaryValue(summary.giftBoxes, "100009");
  const randomGoldPool = summaryValue(summary.equivalentGiftPools, "random-gold");
  const transferPanel = `<section class="inventory-transfer-primary" aria-labelledby="inventory-transfer-title"><div class="inventory-transfer-copy"><h2 id="inventory-transfer-title">${escapeHtml(t(locale, "inventoryTransferTitle"))}</h2><small>${escapeHtml(t(locale, "inventoryImportHint"))}</small></div><div class="inventory-transfer-actions"><button type="button" class="secondary-button" data-export-inventory>${escapeHtml(t(locale, "inventoryExport"))}</button><button type="button" class="primary-button" data-import-inventory>${escapeHtml(t(locale, "inventoryImport"))}</button><input id="inventory-import-file" type="file" accept="application/json,.json" hidden></div></section>`;
  const secondaryTools = `${renderPeriodicResources({ data, state, locale })}${renderGiftBoxes({ data, state, summary, locale })}${renderEquivalentPools({ data, state, summary, locale })}${renderSynthesis({ data, state, locale, localization })}`;
  return `<section class="inventory-workspace" aria-labelledby="inventory-title"><div class="inventory-page-hero"><div><span class="workspace-kicker">${escapeHtml(t(locale, "workbenchInventory"))}</span><h2 id="inventory-title">${escapeHtml(t(locale, "inventoryManagementTitle"))}</h2><p>${escapeHtml(t(locale, "inventoryHint"))}</p></div>${renderInventoryArt({ data, summary, locale, localization })}<div class="inventory-overview"><article><span>${escapeHtml(t(locale, "inventoryCurrent"))}</span><strong>${formatSmartQuantity(currentGiftTotal, locale)}</strong><small>${escapeHtml(t(locale, "inventoryGiftsTitle"))}</small></article><article><span>${escapeHtml(t(locale, "inventoryRemaining"))}</span><strong>${formatSmartQuantity(remainingGiftTotal, locale)}</strong><small>${escapeHtml(t(locale, "inventoryGiftsTitle"))}</small></article><article><span>${escapeHtml(t(locale, "inventoryBoxName", "100008"))}</span><strong>${formatSmartQuantity(choiceBox.current, locale)}</strong><small>100008</small></article><article><span>${escapeHtml(t(locale, "inventoryBoxName", "100009"))}</span><strong>${formatSmartQuantity(randomPurpleBox.current, locale)}</strong><small>100009</small></article><article><span>${escapeHtml(t(locale, "inventoryPoolName", "random-gold"))}</span><strong>${formatSmartQuantity(randomGoldPool.current, locale)}</strong><small>${escapeHtml(t(locale, "inventoryExpectedOnly"))}</small></article></div></div>${transferPanel}${message ? `<div class="inventory-notice" role="status">${escapeHtml(message)}</div>` : ""}${renderReservationPanel({ data, state, summary, locale, localization })}${renderStockResources({ data, state, summary, locale })}${renderGifts({ data, state, summary, locale, localization, filters })}<details class="inventory-more-details"><summary>${escapeHtml(t(locale, "inventoryMoreTitle"))}</summary><div class="inventory-more-content">${secondaryTools}</div></details></section>`;
}
